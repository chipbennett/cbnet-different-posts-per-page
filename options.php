 ...
 * appropriate capability for saving Plugin settings.
 */
function cbnetdppp_get_settings_page_cap() {
	return 'edit_Plugin_options';
}
// Hook into option_page_capability_{option_page}
add_action( 'option_page_capability_cbnetdppp-settings', 'cbnetdppp_get_settings_page_cap' );

/**
 * Setup the Plugin Admin Settings Page
 * 
 * Add "cbnet Different Posts Per Page Options" link to the "Settings" menu
 * 
 * @uses	cbnetdppp_get_settings_page_cap()	defined in \functions\wordpress-hooks.php
 */
function cbnetdppp_add_settings_page() {
	// Globalize Plugin options page
	global $cbnetdppp_settings_page;
	// Add Plugin options page
	$cbnetdppp_settings_page = add_options_page(
		// $page_title
		// Name displayed in HTML title tag
		__( 'cbnetdppp Options', 'cbnetdppp' ), 
		// $menu_title
		// Name displayed in the Admin Menu
		__( 'cbnetdppp Options', 'cbnetdppp' ), 
		// $capability
		// User capability required to access page
		cbnetdppp_get_settings_page_cap(), 
		// $menu_slug
		// String to append to URL after "Plugins.php"
		'cbnetdppp-settings', 
		// $callback
		// Function to define settings page markup
		'cbnetdppp_admin_options_page'
	);
	// Load contextual help
	//add_action( 'load-' . $cbnetdppp_settings_page, 'cbnetdppp_settings_page_contextual_help' );
}
// Load the Admin Options page
add_action( 'admin_menu', 'cbnetdppp_add_settings_page' );

/**
 * cbnetdppp Plugin Settings Page Markup
 * 
 * @uses	cbnetdppp_get_current_tab()	defined in \functions\custom.php
 * @uses	cbnetdppp_get_page_tab_markup()	defined in \functions\custom.php
 */
function cbnetdppp_admin_options_page() { 
	// Define the page section
	$settings_section = 'cbnetdppp';
	?>
	...
	<div class="wrap">
		...
		<?php 
			...
			settings_fields('plugin_cbnetdppp_options');
			// Output each settings section, and each
			// Settings field in each section
			do_settings_sections( $settings_section );
		?>
			<?php submit_button( __( 'Save Settings', 'cbnetdppp' ), 'primary', 'Plugin_cbnetdppp_options[submit]', false ); ?>
			<?php submit_button( __( 'Reset Defaults', 'cbnetdppp' ), 'secondary', 'Plugin_cbnetdppp_options[reset]', false ); ?>
		</form>
	</div>
<?php 
}
